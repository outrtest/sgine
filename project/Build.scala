...
object SgineBuild extends Build {
  ...
  lazy val root = Project("root", file("."), settings = createSettings("sgine-root"))
      ...
      .aggregate(concurrent, core, event, input, math, openglGenerator, openglApi, openglLwjgl,
    openglAndroid, openglNehe,
  ...
  lazy val math = Project("math", file("math"), settings = createSettings("sgine-math"))
      .dependsOn(core)
  lazy val openglGenerator = Project("opengl-generator", file("opengl/generator"),
    settings = createSettings("sgine-opengl-generator"))
      .settings(libraryDependencies ++= Seq(
    "org.lwjgl.lwjgl" % "lwjgl" % lwjglVersion,
    "com.googlecode.reflective" %% "reflective" % "1.0"
  ))
      .dependsOn(core)
  lazy val openglApi = Project("opengl-api", file("opengl/api"),
    settings = createSettings("sgine-opengl-api"))
      .dependsOn(core)
  lazy val openglLwjgl = Project("opengl-lwjgl", file("opengl/lwjgl"),
    settings = createSettings("sgine-opengl-lwjgl"))
      .dependsOn(openglApi)
      .settings(libraryDependencies += "org.lwjgl.lwjgl" % "lwjgl" % lwjglVersion)
  lazy val openglAndroid = Project("opengl-android", file("opengl/android"),
    settings = createSettings("sgine-opengl-android"))
      .dependsOn(openglApi)
  lazy val openglNehe = Project("opengl-nehe", file("opengl/nehe"),
    settings = createSettings("sgine-opengl-nehe"))
