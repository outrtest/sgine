...
class SgineProject(info: ProjectInfo) extends ParentProject(info) {
  ...
  lazy val opengl = project("opengl", "sgine-opengl", new OpenGLProject(_), properties)
  lazy val properties = project("property", "sgine-property")
  ...
  class OpenGLProject(info: ProjectInfo) extends ParentProject(info) {
    ...
    lazy val api = project("api", "sgine-opengl-api", properties)
    lazy val lwjgl = project("lwjgl", "sgine-opengl-lwjgl", new OpenGLLWJGLProject(_), api, properties)
    lazy val android = project("android", "sgine-opengl-android", new OpenGLAndroidProject(_), api, properties)
  }
